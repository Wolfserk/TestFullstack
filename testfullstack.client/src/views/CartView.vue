<!--<template>
  <div class="max-w-4xl mx-auto mt-10 p-6 bg-white rounded-lg shadow-lg">
    ...
    <div class="mt-6 flex justify-between items-center">
      ...
      <button @click="submitOrder" class="bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700">
        Оформить заказ
      </button>
    </div>
  </div>
  ...
  export default defineComponent({
    ...
    setup() {
      ...
      const submitOrder = async () => {
        try {
          await axios.post("https://localhost:7034/api/orders", {
            items: cartStore.cart.map(item => ({ itemId: item.id, quantity: item.quantity })),
            status: "Новый",
          });
          cartStore.clearCart();
      ...
      return { cart: cartStore.cart, total, removeFromCart, submitOrder };
    },
  });
</script>-->
  ...
  export default defineComponent({
    ...
    setup() {
      ...
      const submitOrder = async () => {
        try {
          await axios.post("https://localhost:7034/api/orders", {
            items: cart.value.map(item => ({ itemId: item.id, quantity: item.quantity })),
            status: "Новый",
          });
          cartStore.clearCart();
          cart.value = [];
